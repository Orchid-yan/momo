...
Momo 是一只住在 Windows 桌面上的小猫，外形是 **2.5D 杏橘色绒毛 PNG**（`src/renderer/src/assets/momo-idle.png`，透明底），不是手绘 SVG。她会一直待在最上层、没有窗口边框、背景透明。**按住拖动**可以放到桌面任意位置；**单击**会撒娇（打滚、心心眼等）；**双击**打开和通义千问的聊天框。
...
| 单击（几乎不移动） | 播放约 1.5 秒的撒娇动画（打滚 / 心心眼 / 伸懒腰 / 招手 / 眨眼闪光），并冒出「喵～」「再摸摸」等小气泡。为了让双击能打开聊天，单击动画会略等约 0.4 秒再开始 |
